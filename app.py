...
app.layout = html.Div([
    ...
    # Graph for PF Projection
    dcc.Graph(id="pf-graph"),
])

# Callback to calculate and update results
@app.callback(
    [Output("output-summary", "children"), Output("pf-graph", "figure")],
...
def update_dashboard(n_clicks, salary, existing_pf, years, hike):
    if n_clicks == 0:
        return "", px.scatter()  # No graph before clicking
    ...
    # Create Graph
    fig = px.line(df, x="Year", y="Total PF Balance", title="Projected PF Growth Over Time",
                  labels={"Year": "Years", "Total PF Balance": "PF Balance (₹)"}, markers=True)
